      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mt-1">
            MAPA DA <span className="text-cyan-500">ROTA</span>
          </h2>
        ...
        <div className="bg-white rounded-3xl shadow-lg overflow-hidden border border-[#e0e8f0]">
          <div className="grid lg:grid-cols-5 gap-0">
            {/* Map visual */}
            <div className="lg:col-span-3 relative min-h-[420px] bg-gradient-to-br from-slate-100 to-slate-200 overflow-hidden">
            ...
            <div className="lg:col-span-2 p-6 sm:p-8 flex flex-col justify-center">
              <div className="inline-flex items-center gap-2 bg-slate-900/8 px-3 py-1 rounded-full mb-6 w-fit">
                <Navigation2 className="h-3.5 w-3.5 text-slate-900" />
                  ...
                  return (
                    <div key={stop.name} className="flex items-start gap-4">
                      ...
                      <div className="flex flex-col items-center">
                        <div
                          className={`w-4 h-4 rounded-full border-[3px] flex-shrink-0 ${isFirst || isLast
                              ? "border-cyan-500 bg-cyan-500"
                              : "border-violet-500 bg-white"
                            }`}
                        />
                        {!isLast && (
                          <div className="w-0.5 h-10 bg-gradient-to-b from-violet-500/40 to-violet-500/10" />
                        )}
                      </div>
                      {/* Info */}
                      <div className={`pb-4 ${isLast ? "pb-0" : ""}`}>
                        <p className={`text-sm font-extrabold ${isFirst || isLast ? "text-slate-900" : "text-slate-900/70"}`}>
                          {stop.name}
                        </p>
                        <p className="text-xs text-slate-500 font-medium">{stop.sub}</p>
                        {stop.time && (
                          <span className="inline-flex items-center gap-1 text-[10px] font-bold text-cyan-500 mt-1">
              ...
              <div className="mt-6 bg-slate-50 rounded-xl p-4">
                <p className="text-[10px] font-bold text-slate-900/50 uppercase tracking-wider mb-1">Informações</p>
                <ul className="space-y-1.5">
                  {ROUTE_INFO_ITEMS.map((item) => (
                    <li key={item.text} className="flex items-center gap-2 text-xs text-slate-900 font-medium">
