...
@trip_router.get("/")
async def get_all_trips(service: TripService = Depends(get_trip_service), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
    result = await service.get_all()
    return ResponseHandler.ok(result)

@trip_router.get("/reservations")
async def get_all_reservations(service: TripService = Depends(get_trip_service), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
...
async def cancel_trip(trip_id: uuid.UUID, controller: TripController = Depends(get_trip_controller), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
    result = await controller.cancel_trip(trip_id)
    return ResponseHandler.ok(result)

@trip_router.get("/{trip_id}")
async def get_trip(trip_id: uuid.UUID, service: TripService = Depends(get_trip_service),  _: TokenData = Depends(require_profile(UserProfile.ADMIN, UserProfile.DRIVER, UserProfile.STAFF, UserProfile.STUDENT))):
    result = await service.get_by_id(trip_id)
    return ResponseHandler.ok(result)

@trip_router.post("/")
async def create_trip(data: CreateTripDTO, service: TripService = Depends(get_trip_service), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
    result = await service.create(data)
    return ResponseHandler.created(result)

@trip_router.patch("/{trip_id}")
async def patch_trip(trip_id: uuid.UUID, data: UpdateTripDTO, service: TripService = Depends(get_trip_service), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
...
async def delete_trip(trip_id: uuid.UUID, service: TripService = Depends(get_trip_service), _: TokenData = Depends(require_profile(UserProfile.ADMIN))):
